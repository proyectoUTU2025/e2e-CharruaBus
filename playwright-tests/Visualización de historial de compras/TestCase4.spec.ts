...
test('Test Case 4: No hay compras que coincidan con los filtros aplicados', async ({ page }) => {
  ...
  //5.Seleccionar la opción "Mis compras" en la interfaz
  await page.getByText('historyMis ComprasVer').click();

  //6.Aplicar filtros que se sabe que no arrojarán ningún resultado
  await page.getByText('Monto Mínimo').click();
  await page.getByRole('spinbutton', { name: 'Monto Mínimo' }).fill('1500');
